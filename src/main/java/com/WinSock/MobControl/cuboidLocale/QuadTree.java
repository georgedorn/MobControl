package com.WinSock.MobControl.cuboidLocale;
...
public class QuadTree {
	...
	public BookmarkedResult relatedSearch(QuadNode bookmark, long x, long y,
			long z) {
		if (bookmark == null) {
			bookmark = root;
		}
	 ...
	 * and upper right possibilities need be regarded.
	 * 
	 * Spits out a list of cuboids that are fit for "insertion" although we just
	 * use them for the search and actually attach the original cuboid.
	 * 
	 * We also return the remainder shard if we generated any others. At the
	 * other end we only include a node if it's shard didn't re-shard. Keeps the
	 * tree search spaces minimal.
	 * 
	 */
	private ArrayList<PrimitiveCuboid> generateShards(QuadNode node,
		...
		if (top < c.xyzB[2]) {
			...
			shards.add(new PrimitiveCuboid(c.xyzA[0], 0, top + 1, tmp, 0,
					c.xyzB[2]));
		}
		if (right < c.xyzB[0]) {
			if (top < c.xyzB[2]) {
